...
export async function fetchFlightLogsForDrone(droneId, pageNumber = 1) {
  ...
  try {
    ...
    // Use POST endpoint for pagination if page > 1, otherwise use GET for page 1
    if (pageNumber > 1) {
      console.log(`📤 Using POST endpoint for page ${pageNumber}`)
      response = await fetch(`/api/drones/detail-page`, {
        method: 'POST',
        headers: {
      ...
      console.log(`📥 Using GET endpoint for page 1`)
      response = await fetch(`/api/drones/detail?id=${encodeURIComponent(droneId)}`)
    }
